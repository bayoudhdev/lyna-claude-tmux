...
func FuzzPlaceAgent(f *testing.F) {
	...
	f.Add(80, 24, 2, 1, 3)
	f.Add(0, 0, 9, 0, 2)
	f.Fuzz(func(t *testing.T, width, height, teammates, others, maxPanes int) {
		w := layout.AgentWindow{Width: width, Height: height, Teammates: teammates, Others: others, Max: maxPanes}
		...
		column := width - width*layout.AgentLeadRatio/100 - 1
		each := (height - (teammates - 1)) / teammates
