...
func TestTaskCheckPrompt(t *testing.T) {
	cases := []struct {
		name    string
		prompt  string
		wantErr string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			err := taskCheckPrompt(tc.prompt)
			if tc.wantErr == "" {
				if err != nil {
					t.Fatalf("taskCheckPrompt(%q) = %v", tc.prompt, err)
				}
				return
			}
			if !errors.Is(err, ErrPrompt) || !strings.Contains(err.Error(), tc.wantErr) {
				t.Fatalf("taskCheckPrompt(%q) = %v, want %q", tc.prompt, err, tc.wantErr)
			}
		})
	}
}
